...
__global__
static void audiofir_kernel(
  float *yout, float *yin, float *coeff, int n, int len)
{
  /* Tu trzeba b?dzie wstawi? prawdziwy kod... */
  ...
  float *yout, float *yin, float *coeff, int n, int len,...)
{
  ...
  checkCudaErrors(cudaMemcpy(filter, coeff, (n+1)*sizeof(float), cudaMemcpyHostToDevice));
  checkCudaErrors(cudaMemcpy(y_in, yin, (2*len)*sizeof(float), cudaMemcpyHostToDevice));
  ...
  audiofir_kernel<<<(len+K-1)/K, K >>>(y_out+len, y_in+len, filter, n,  len);
  checkCudaErrors(cudaGetLastError());
  checkCudaErrors(cudaDeviceSynchronize());

  checkCudaErrors(cudaMemcpy(yout, y_out, (2*len)*sizeof(float), cudaMemcpyDeviceToHost));
