...
/**********************************************************/
void alloc_mem(int n, int len, float **coeff_ptr, float **yin_ptr,
...
void free_mem(float *coeff_ptr, float *yin_ptr, float *yref_ptr,
              float *yout_ptr) {
  free(coeff_ptr);
  free(yin_ptr);
  free(yref_ptr);
...
void audiocmp(float *yout, float *yref, int len) {
  ...
  float d, e = -1.0f;
  for (k = 0; k < 2 * len; k++)
    if ((d = fabsf(yout[k] - yref[k])) > e) e = d;
...
int main(int argc, char *argv[]) {
  ...
  timer(&start);
  audiofir::audiofir(yout, yin, coeff, n, len, argc, argv);
  timer(&stop);
